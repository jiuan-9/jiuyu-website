...
// ─── Shared input styles ───
const inputClass =
  "w-full bg-dark-800/80 border border-white/[0.12] rounded-lg px-3 py-2 text-xs text-dark-100 outline-none focus:border-brand-500/50 focus:ring-1 focus:ring-brand-500/20 placeholder:text-dark-500 transition-colors";
const selectClass =
  "w-full bg-dark-800/80 border border-white/[0.12] rounded-lg px-3 py-2 text-xs text-dark-100 outline-none focus:border-brand-500/50 focus:ring-1 focus:ring-brand-500/20 appearance-none cursor-pointer transition-colors";
const labelClass = "text-[10px] text-dark-300 font-medium";
const hintClass = "text-[9px] text-dark-500";

// ─── Section header ───
function SectionHeader({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="flex items-center gap-2 pt-3 pb-1.5 border-t border-white/[0.06]">
      {icon}
...
function buildSystemPrompt(
  ...
  if (userParts.length) parts.push(`正在与你对话的用户：${userParts.join("，")}。`);
  ...
  const sidebarContent = (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 text-xs">
        {/* ─── 当前会话 ─── */}
        <SectionHeader icon={<FileText size={11} className="text-brand-400" />} label="当前会话" />
        <div className="flex gap-3 py-1.5">
          <div className="flex-1 bg-dark-800/80 border border-white/[0.08] rounded-lg px-3 py-2 text-center">
            <div className="text-[10px] text-dark-400">对话轮数</div>
            <div className="text-sm font-semibold text-dark-100">{roundCount}</div>
          </div>
          <div className="flex-1 bg-dark-800/80 border border-white/[0.08] rounded-lg px-3 py-2 text-center">
            <div className="text-[10px] text-dark-400">上下文</div>
            <div className="text-sm font-semibold text-dark-100">{contextLimit} 条</div>
          </div>
        </div>

        {/* ─── API 设置 ─── */}
        <SectionHeader icon={<Key size={11} className="text-brand-400" />} label="API 设置" />
        ...
        {/* ─── AI 设定 ─── */}
        <SectionHeader icon={<Sparkles size={11} className="text-brand-400" />} label="AI 设定" />

        <div className="flex items-center justify-between py-1">
          <span className="text-[10px] text-dark-300">精调人设</span>
          <button onClick={() => setCompileEnabled(!compileEnabled)} className="transition-colors">
        ...
        {/* ─── 用户设定 ─── */}
        <SectionHeader icon={<User size={11} className="text-brand-400" />} label="用户设定" />
        ...
        {/* ─── 场景设定 ─── */}
        <SectionHeader icon={<MapPin size={11} className="text-brand-400" />} label="场景设定" />
  ...
  return (
    <div className="h-screen flex flex-col bg-dark-950 text-dark-200 overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {/* ─── Desktop sidebar (inline flex) ─── */}
        <div
          className={`hidden md:flex ${
            sidebarOpen ? "w-[312px]" : "w-0"
          } shrink-0 border-r border-white/[0.06] bg-dark-900/80 flex-col transition-[width] duration-300 overflow-hidden`}
        >
          {sidebarContent}
        </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-5">
            {messages.map((msg, i) => (
              <div key={i} className={`flex items-start gap-2 sm:gap-3 ${msg.role === "user" ? "justify-end" : ""}`}>
                ...
                <div
                  className={`p-3 sm:p-3.5 rounded-2xl text-sm leading-relaxed max-w-[88%] sm:max-w-[85%] whitespace-pre-wrap break-words ${
                    msg.role === "user"
                      ? "rounded-tr-sm bg-brand-500/10 border border-brand-500/15 text-dark-100"
                      : "rounded-tl-sm bg-dark-900/80 border border-white/[0.06] text-dark-200"
                  }`}
                >
                  {msg.content}
                </div>
            ...
            {loading && (
              <div className="flex items-start gap-2 sm:gap-3">
                ...
                <div className="p-3 sm:p-3.5 rounded-2xl rounded-tl-sm bg-dark-900/80 border border-white/[0.06]">
                  <LoadingDots />
                </div>
              </div>
            )}
